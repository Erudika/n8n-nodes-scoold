...
function buildPaginationQs(fields) {
    const qs = {};
    if (fields.limit !== undefined)
        qs.limit = fields.limit;
    if (fields.page !== undefined)
        qs.page = fields.page;
    if (fields.sortby)
        qs.sortby = fields.sortby;
            ...
            try {
                ...
                if (resource === 'search' && operation === 'query') {
                    ...
                    if (returnAll) {
                        let lastKey;
                        do {
                            const qs = { desc: descending };
                            if (sortBy)
                                qs.sortby = sortBy;
                            if (lastKey)
                                qs.lastKey = lastKey;
                            const response = (await this.helpers.httpRequestWithAuthentication.call(this, 'scooldApi', { method: 'GET', url, qs, json: true }));
                        ...
                        if (sortBy)
                            qs.sortby = sortBy;
                        envelope = (await this.helpers.httpRequestWithAuthentication.call(this, 'scooldApi', { method: 'GET', url, qs, json: true }));
                    ...
                    if (operation === 'create') {
                        ...
                        if (postType === 'reply' && !parentId) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Parent Post ID is required when Post Type is Reply', { itemIndex: i });
                        }
                        const bodyData = { type: postType, title };
                        if (body)
                            bodyData.body = body;
                        if (tags)
                            bodyData.tags = tags.split(',').map((t) => t.trim()).filter(Boolean);
                        if (parentId)
                            bodyData.parentid = parentId;
                        if (additionalFields.creatorid)
                            bodyData.creatorid = additionalFields.creatorid;
                        if (additionalFields.space)
                            bodyData.space = additionalFields.space;
                        if (additionalFields.wiki !== undefined)
                            bodyData.wiki = additionalFields.wiki;
                        if (additionalFields.location)
                            bodyData.location = additionalFields.location;
                        const qs = {};
                        if (additionalFields.notificationsDisabled)
                            qs.notificationsDisabled = true;
                        ...
                        if (additionalFields.title)
                            bodyData.title = additionalFields.title;
                        if (additionalFields.creatorid)
                            bodyData.creatorid = additionalFields.creatorid;
                        if (additionalFields.lasteditby)
                            bodyData.lasteditby = additionalFields.lasteditby;
                        if (additionalFields.space)
                            bodyData.space = additionalFields.space;
                        if (additionalFields.wiki !== undefined)
                            bodyData.wiki = additionalFields.wiki;
                        ...
                        if (additionalFields.space)
                            qs.space = additionalFields.space;
                        if (additionalFields.q)
                            qs.q = additionalFields.q;
                        if (additionalFields.includeReplies)
                            qs.includeReplies = additionalFields.includeReplies;
                        ...
                        if (additionalFields.desc !== undefined)
                            qs.desc = additionalFields.desc;
                        const response = await this.helpers.httpRequestWithAuthentication.call(this, 'scooldApi', {
                            method: 'GET',
                            url: `${baseUrl}/api/posts`,
                            qs,
                    ...
                    if (operation === 'create') {
                        ...
                        const parentId = this.getNodeParameter('parentId', i);
                        const additionalFields = this.getNodeParameter('additionalFields', i);
                        const bodyData = { comment: commentText, parentid: parentId };
                        if (additionalFields.creatorid)
                            bodyData.creatorid = additionalFields.creatorid;
                        const response = (await this.helpers.httpRequestWithAuthentication.call(this, 'scooldApi', {
                            method: 'POST',
                    ...
                    if (operation === 'create') {
                        ...
                        const bodyData = {};
                        if (updateFields.tag)
                            bodyData.tag = updateFields.tag;
                        if (updateFields.description)
                            bodyData.description = updateFields.description;
                        const response = (await this.helpers.httpRequestWithAuthentication.call(this, 'scooldApi', {
                            method: 'PATCH',
                            url: `${baseUrl}/api/tags/${tagId}`,
                            body: bodyData,
                    ...
                    if (operation === 'create') {
                        ...
                        const reportId = this.getNodeParameter('reportId', i);
                        const solution = this.getNodeParameter('solution', i);
                        const response = (await this.helpers.httpRequestWithAuthentication.call(this, 'scooldApi', {
                            method: 'PUT',
                        ...
                        returnData.push({
                            json: response !== null && response !== void 0 ? response : { success: true, id: reportId },
                            pairedItem: { item: i },
                        });
                    }
                    else {
